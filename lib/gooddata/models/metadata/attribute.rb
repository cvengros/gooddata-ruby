...
module GoodData
  class Attribute < MdObject
    ...
    def create_metric(options = {})
      ...
      if an_attribute
        Metric.xcreate("SELECT #{a_type.to_s.upcase}([#{uri}], [#{an_attribute.uri}])", :title => a_title, :client => client, :project => project, :extended_notation => false)
      else
        Metric.xcreate("SELECT #{a_type.to_s.upcase}([#{uri}])", :title => a_title, :client => client, :project => project, :extended_notation => false)
      end
    end
